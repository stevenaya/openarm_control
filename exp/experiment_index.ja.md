...
全 controller-comparison video は profile/parameter、`arm_origin` 座標、playback multiplier、command/state/elbow path legend を表示する。赤い座標軸が target、半透明の腕が IK または driver command、不透明の腕が simulated actual state である。
